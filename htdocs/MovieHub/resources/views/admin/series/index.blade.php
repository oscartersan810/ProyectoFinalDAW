...
<main class="min-h-screen bg-gradient-to-br from-gray-900 to-black flex items-center justify-center p-6">
    ...
    <div class="absolute inset-0 overflow-hidden opacity-10">
        <div class="particle" style="top: 25%; left: 15%; width: 4px; height: 4px;"></div>
    ...
    <div class="relative bg-gray-900 rounded-3xl shadow-2xl p-8 md:p-12 w-full max-w-6xl border border-yellow-500 backdrop-blur-sm overflow-hidden">
        ...
        @if($series->isEmpty())
            <div class="text-center py-12">
                ...
                <p class="text-xl text-gray-400">No hay series registradas</p>
                <p class="text-gray-500 mt-2">Comienza agregando una nueva serie</p>
            </div>
        @else
                        ...
                        <tr class="hover:bg-gray-750 transition duration-150">
                            <td class="px-4 py-3">
                                <div class="flex-shrink-0 h-16 w-12">
                                    @if($serie->poster)
                                        <img class="h-16 w-12 rounded-lg object-cover border border-gray-600 shadow" src="{{ asset('storage/' . $serie->poster) }}" alt="Póster">
                                    @else
                            ...
                            <td class="px-4 py-3">
                                <div class="text-sm font-medium text-white">{{ $serie->title }}</div>
                            </td>
                            <td class="px-4 py-3 hidden md:table-cell">
                                <div class="text-sm text-gray-300 line-clamp-2" title="{{ $serie->description }}">
                                    {{ $serie->description }}
                                </div>
                            </td>
                            <td class="px-4 py-3">
                            ...
                            <td class="px-4 py-3">
                                <div class="text-sm text-gray-300">{{ $serie->year }}</div>
                            </td>
                            <td class="px-4 py-3 text-right text-sm font-medium">
                                <div class="flex justify-end space-x-2">
                                    <a href="{{ route('admin.series.edit', $serie->id) }}" class="text-yellow-400 hover:text-yellow-300 transition flex items-center" title="Editar">
                                        <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                    ...
                                    <form action="{{ route('admin.series.destroy', $serie->id) }}" method="POST" class="inline">
                                        @csrf
                                        @method('DELETE')
                                        <button type="submit" onclick="return confirm('¿Estás seguro de eliminar esta serie?')" 
